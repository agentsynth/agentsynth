...
- **Reward-hacking resistance** — `agentsynth pack audit` measures how gameable a pack's checkers are with trivial adversaries (a canned answer, an echoed prompt, a throwaway call) and flags answers that leaked into the prompt, so a pack ships with a known robustness floor instead of a hidden one. [We ran it on our own packs](docs/ROBUSTNESS_REPORT.md), weaknesses included.
